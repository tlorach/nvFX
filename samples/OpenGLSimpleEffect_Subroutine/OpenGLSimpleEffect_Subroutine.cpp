...
bool loadMaterialEffect()
{
    ...
    fx_Effect = nvFX::IContainer::create("effectSimple1");
    bool bRes = nvFX::loadEffectFromFile(fx_Effect, "effect_subroutine.glslfx");
    if(!bRes)
        bRes = nvFX::loadEffectFromFile(fx_Effect, SOURCE_DIR "/effect_subroutine.glslfx");
    if(!bRes)
    {
        LOGE("Failed\n");
    }
    else
    {
        ...
        fx_Tech = fx_Effect->findTechnique(TECH_DEFAULT+1);
