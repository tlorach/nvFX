...
void loadModel()
{
    ...
    //---------------
    LOGI("Loading Meshe... %s\n", MODEL);
    g_meshFile = bk3d::load(MODEL);
    if(!g_meshFile)
    {
        LOGI("Trying " RESOURCE_DIR "/%s\n", MODEL);
        char fullname[200];
        sprintf(fullname, RESOURCE_DIR "/%s", MODEL);
        g_meshFile = bk3d::load(fullname);
    }
    //// use tech #2
    ...
    for(int i=0; i<g_meshFile->pMeshes->n; i++)
    {
        bk3d::Mesh *pMesh = g_meshFile->pMeshes->p[i];
